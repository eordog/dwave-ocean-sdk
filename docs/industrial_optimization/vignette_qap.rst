...
D-Wave's hybrid nonlinear-program (NL) solver can efficiently encode a QAP
problem by taking advantage of a list variable that encodes an ordering of the
facility placements. This variable eliminates the need for both the quadratic
number of binary variables representing facility placements and the constraints
preventing multiple facilities per location (and vice versa) required by the
MINLP formulation. The arrays of flows and distances are converted to constant
variables from which the objective value is computed.
